    ...
    $.ajax({
        ...
        success: function(data){
            if(data.res.toLowerCase() == 'ok'){
              ...
              $('li[data-path="'+item.fullPath+'"] .icon').attr('src', '/static/roxyfileman/' + RoxyUtils.GetFileIcon(newName));
...
function switchView(t){
  ...
  if(t == 'thumb'){
    ...
    $('#pnlFileList li').each(function(){
      $(this).children('.icon').css('background-image','').attr('src', '/static/roxyfileman/' + $(this).attr('data-icon'));
